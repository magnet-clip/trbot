...
def publish_url(bot: Bot, update: Update, user_data):
    ...
    bot.send_message(chat_id=update.message.chat_id, text="Спасибо за урл! Укажите чат:", reply_markup=markup)
    return ChooseChats
...
def publish_chats(bot: Bot, update: Update, user_data):
    ...
    bot.send_message(chat_id=update.message.chat_id, text="You selected {}".format(channel))
    if channel not in channels:
        bot.send_message(chat_id=update.message.chat_id, text="No such channel")
        return ConversationHandler.END
    else:
        bot.send_message(chat_id=update.message.chat_id, text="Such channel")
        options = [["Yes"], ["No"]]
        bot.send_message(chat_id=update.message.chat_id, text="PPublish?",
